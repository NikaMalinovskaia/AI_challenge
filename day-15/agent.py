from __future__ import annotations
import json
import os
from openai import OpenAI

class GuardedFSMAgent:
    def __init__(self, storage_dir="memory_storage_day15", model_name="glm-4.7-flash", temperature=0.3):
        self.storage_dir = storage_dir
        self.model_name = model_name
        self.temperature = temperature
        os.makedirs(self.storage_dir, exist_ok=True)
        
        api_key = os.environ.get("LITELLM_API_KEY") or "sk-L2Fx4Xsvy_OA6gcp3gG2pA"
        
        self.client = OpenAI(
            api_key=api_key,
            base_url="https://llm.effective.land/v1",
        )

        # Матрица разрешенных переходов (граф состояний)
        self.ALLOWED_TRANSITIONS = {
            "planning": ["execution"],
            "execution": ["planning", "validation"], # можно вернуть на доработку или идти дальше
            "validation": ["execution", "done"],     # можно вернуть на исправление ошибок или завершить
            "done": []                               # финал, дальше переходов нет
        }

        initial_structures = {
            "short_term": [],
            "working": {
                "stage": "planning",
                "current_step": "Сбор требований и выбор стека",
                "expected_action": "Утвердить план и стек у пользователя"
            },
            "invariants": {
                "allowed_stack": ["FastAPI", "PostgreSQL", "SQLAlchemy"],
                "strict_lifecycle": True
            }
        }
        
        for mem_type, default_val in initial_structures.items():
            path = self._get_path(mem_type)
            if not os.path.exists(path):
                self.save_memory(mem_type, default_val)

    def _get_path(self, memory_type: str) -> str:
        return os.path.join(self.storage_dir, f"{memory_type}.json")

    def load_memory(self, memory_type: str) -> dict | list:
        path = self._get_path(memory_type)
        if not os.path.exists(path):
            return [] if memory_type == "short_term" else {}
        with open(path, "r", encoding="utf-8") as f:
            return json.load(f)

    def save_memory(self, memory_type: str, data):
        path = self._get_path(memory_type)
        with open(path, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)

    def change_stage(self, new_stage: str) -> tuple[bool, str]:
        """Строгая проверка и выполнение перехода в рамках FSM."""
        working = self.load_memory("working")
        current_stage = working.get("stage", "planning")

        if current_stage == new_stage:
            return True, f"Уже находимся на этапе {current_stage}."

        allowed_next = self.ALLOWED_TRANSITIONS.get(current_stage, [])

        if new_stage not in allowed_next:
            return False, (
                f"🛑 НЕДОПУСТИМЫЙ ПЕРЕХОД: Нельзя перейти из этапа '{current_stage}' "
                f"в этап '{new_stage}'. Разрешены только переходы в: {allowed_next}."
            )

        # Если переход разрешен — обновляем этап
        working["stage"] = new_stage
        if new_stage == "execution":
            working["current_step"] = "Написание кода по утвержденному плану"
            working["expected_action"] = "Реализовать эндпоинты"
        elif new_stage == "validation":
            working["current_step"] = "Тестирование и проверка"
            working["expected_action"] = "Проверить код на ошибки"
        elif new_stage == "done":
            working["current_step"] = "Проект завершен"
            working["expected_action"] = "Сдача результата"

        self.save_memory("working", working)
        return True, f"Этап успешно изменен с '{current_stage}' на '{new_stage}'."

    def count_tokens(self, text: str) -> int:
        return max(1, len(text) // 4)

    def send_message(self, user_message: str, keep_last_n: int = 6):
        short_term = self.load_memory("short_term")
        working = self.load_memory("working")
        invariants = self.load_memory("invariants")

        short_term.append({"role": "user", "content": user_message})

        current_stage = working.get("stage", "planning")
        allowed_next = self.ALLOWED_TRANSITIONS.get(current_stage, [])

        system_prompt = (
            "Ты — строгий архитектор, контролирующий жизненный цикл задачи через конечный автомат (FSM).\n\n"
            f"⚙️ ТЕКУЩЕЕ СОСТОЯНИЕ FSM:\n{json.dumps(working, ensure_ascii=False, indent=2)}\n\n"
            f"🔄 СТРОГО РАЗРЕШЕННЫЕ СЛЕДУЮЩИЕ ЭТАПЫ из текущего ({current_stage}): {allowed_next}\n\n"
            f"🛡️ ИНВАРИАНТЫ ЖИЗНЕННОГО ЦИКЛА:\n{json.dumps(invariants, ensure_ascii=False, indent=2)}\n\n"
            "Инструкция:\n"
            "1. Запрещено перепрыгивать этапы жизненного цикла (например, из planning сразу в validation или done).\n"
            "2. Если пользователь просит выполнить действие недопустимого этапа, откажи и объясни ограничения FSM."
        )

        messages_for_api = [{"role": "system", "content": system_prompt}]
        recent_messages = short_term[-keep_last_n:]
        messages_for_api.extend(recent_messages)

        try:
            response = self.client.chat.completions.create(
                model=self.model_name,
                messages=messages_for_api,
                temperature=self.temperature
            )
        except Exception as e:
            short_term.pop()
            self.save_memory("short_term", short_term)
            raise Exception(f"Ошибка API: {e}")

        assistant_reply = response.choices[0].message.content

        short_term.append({"role": "assistant", "content": assistant_reply})
        self.save_memory("short_term", short_term)

        full_context_str = system_prompt + "".join([m["content"] for m in recent_messages])
        req_tokens = self.count_tokens(user_message)
        hist_tokens = self.count_tokens(full_context_str)
        resp_tokens = self.count_tokens(assistant_reply)

        return {
            "reply": assistant_reply,
            "request_tokens": req_tokens,
            "history_tokens": hist_tokens,
            "response_tokens": resp_tokens,
            "cumulative_tokens": hist_tokens + resp_tokens
        }

    def reset_all_memory(self):
        self.save_memory("short_term", [])
        self.save_memory("working", {
            "stage": "planning",
            "current_step": "Сбор требований и выбор стека",
            "expected_action": "Утвердить план и стек у пользователя"
        })
        self.save_memory("invariants", {
            "allowed_stack": ["FastAPI", "PostgreSQL", "SQLAlchemy"],
            "strict_lifecycle": True
        })
